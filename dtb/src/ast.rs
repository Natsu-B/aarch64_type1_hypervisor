...
impl<'dtb> DeviceTree<'dtb> {
    ...
    ///
    /// The returned `Box<[u8]>` owns the DTB bytes.
    /// The AST can be dropped afterwards to free its heap.
    pub fn into_dtb_box(self) -> Result<Box<[u8]>, &'static str> {
...
mod tests {
    ...
    fn parse_real_dtb_fixture() {
        let dtb = include_bytes!("../test/test.dtb");
